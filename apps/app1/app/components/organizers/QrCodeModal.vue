<template>
  <UModal
    v-model:open="isOpen"
    :title="$t('edition.ticketing.qr_modal_title_organizer')"
    :ui="{ width: 'sm:max-w-md' }"
  >
    <template #body>
      <div v-if="editionOrganizerId" class="space-y-4">
        <!-- Instructions -->
        <div
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div class="text-sm text-blue-700 dark:text-blue-300">
              ...
              <p>
                {{ $t('edition.ticketing.qr_instructions_organizer') }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div v-if="editionOrganizer" class="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
          ...
          <div class="space-y-2 text-sm">
            <div class="flex items-center gap-2 text-gray-600 dark:text-gray-400">
              ...
              <span>{{ getEditionDisplayName(editionOrganizer.edition) }}</span>
            </div>
            <div
              v-if="editionOrganizer.organizer?.title"
              class="flex items-center gap-2 text-gray-600 dark:text-gray-400"
            >
              <UIcon name="i-heroicons-shield-check" class="w-4 h-4" />
              <span>{{ editionOrganizer.organizer.title }}</span>
            </div>
          </div>
        </div>
      </div>
...
const downloadQrCode = async () => {
  ...
  link.download = `qr-code-organizer-${editionOrganizerId.value}.png`
