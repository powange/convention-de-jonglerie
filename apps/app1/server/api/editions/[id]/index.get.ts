    ...
    if (edition?.convention?.organizers) {
      edition.convention.organizers = edition.convention.organizers.map((collab) => ({
        ...
        rights: {
          ...
          manageArtists: collab.canManageArtists ?? false,
          manageMeals: collab.canManageMeals ?? false,
          manageTicketing: collab.canManageTicketing ?? false,
          manageWorkshops: collab.canManageWorkshops ?? false,
          manageFAQ: collab.canManageFAQ ?? false,
          manageTreasury: collab.canManageTreasury ?? false,
        ...
        perEditionRights: (collab.perEditionPermissions || []).map((per) => ({
          ...
          canManageArtists: per.canManageArtists ?? false,
          canManageMeals: per.canManageMeals ?? false,
          canManageTicketing: per.canManageTicketing ?? false,
          canManageWorkshops: per.canManageWorkshops ?? false,
          canManageFAQ: per.canManageFAQ ?? false,
          canManageTreasury: per.canManageTreasury ?? false,
