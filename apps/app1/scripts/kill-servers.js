...
async function killServers() {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      // Windows
      try {
        const { stdout: winstdout } = await execAsync(
    ...
    for (const line of lines) {
      ...
      if (
        line.includes('convention-de-jonglerie') ||
    ...
    const uniqueProcesses = processesToKill.filter(
      (proc, index, self) => index === self.findIndex((p) => p.pid === proc.pid)
    )
