<template>
  <UModal
    v-model:open="isOpen"
    :title="group ? $t('gestion.stock.edit_group') : $t('gestion.stock.new_group')"
  >
    <template #body>
      <form class="space-y-4" @submit.prevent="handleSubmit">
        <UFormField :label="$t('gestion.stock.group_name')" required :error="fieldErrors.name">
          <UInput
            v-model="formData.name"
            :placeholder="$t('gestion.stock.group_name_placeholder')"
            class="w-full"
          />
        </UFormField>
        <UFormField :label="$t('gestion.stock.group_description')" :error="fieldErrors.description">
          <UTextarea
            v-model="formData.description"
            :placeholder="$t('gestion.stock.group_description_placeholder')"
            :rows="3"
            class="w-full"
          />
        </UFormField>
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.group) {
      ...
      await $fetch(`/api/editions/${props.editionId}/stock-groups`, {
        method: 'POST',
        body,
      })
    }
