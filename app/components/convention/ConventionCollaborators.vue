  ...
  <UCard>
    ...
    <div v-else-if="collaborators.length === 0" class="text-center py-8">
      ...
      <p class="text-sm text-gray-400">
        Ajoutez des collaborateurs pour qu'ils puissent modifier cette convention
      </p>
    </div>
    ...
    <div v-else class="space-y-3">
      ...
      >
        <div class="flex items-center gap-3">
          <UserAvatar :user="collaborator.user" size="lg" border />
          <div>
            <p class="font-medium">{{ collaborator.user.pseudo }}</p>
            <p class="text-sm text-gray-500">
              {{ collaborator.user.prenom }} {{ collaborator.user.nom }}
            </p>
            <p class="text-xs text-gray-400">
              Ajouté par {{ collaborator.addedBy.pseudo }} le
      ...
      <UCard>
        ...
        <UForm :state="addForm" :schema="addSchema" class="space-y-4" @submit="addCollaborator">
          ...
          <UFormField :label="t('components.collaborators_modal.permissions')" name="canEdit">
            <UCheckbox
              v-model="addForm.canEdit"
              :label="t('components.collaborators_modal.allow_modification')"
            />
          ...
          <div class="flex justify-end space-x-2">
            <UButton type="button" variant="ghost" @click="showAddModal = false"> Annuler </UButton>
            <UButton type="submit" :loading="adding" color="primary"> Ajouter </UButton>
          </div>
        </UForm>
      </UCard>
    </UModal>
