    ...
    <div class="w-full max-w-lg">
      ...
      <UCard class="shadow-xl border-0 backdrop-blur-sm bg-white/80 dark:bg-gray-800/80">
        ...
        <template v-if="step === 'email'">
          <UForm :state="emailState" :schema="emailSchema" class="space-y-6" @submit="handleEmailSubmit">
        ...
        <template v-else-if="step === 'password'">
          <div class="mb-4 text-sm text-gray-600 dark:text-gray-400">
            ...
            <UButton variant="link" color="neutral" size="xs" class="ml-2" @click="step='email'">{{ $t('common.edit') || 'Modifier' }}</UButton>
          </div>
          <UForm :state="passwordState" :schema="passwordSchema" class="space-y-6" @submit="handlePasswordSubmit">
        ...
        <template v-else>
          ...
          <UForm :state="registerState" :schema="registerSchema" class="space-y-6" @submit="handleRegisterSubmit">
                ...
                >
                  <template #trailing>
                    <UButton
                      ...
                      :aria-label="showRegisterConfirm ? t('auth.hide_password') : t('auth.show_password')"
...
const handleEmailSubmit = async () => {
  loading.value = true
  try {
    const res = await $fetch('/api/auth/check-email', { method: 'POST', body: { email: emailState.email } }) as { exists: boolean }
  ...
  try {
    ...
    if ((error as any).statusCode === 401 || (error as any).status === 401) {
      ...
      if (isEmailNotVerified) {
        ...
        toast.add({ title: t('errors.email_not_verified'), description: t('errors.email_verification_required'), icon: 'i-heroicons-envelope-open', color: 'warning' })
...
const handleRegisterSubmit = async () => {
  ...
  try {
    ...
    if (response?.requiresVerification) {
      await navigateTo(`/verify-email?email=${encodeURIComponent(response.email || emailState.email)}`)
      toast.add({ title: t('messages.account_created'), description: t('messages.verification_code_sent'), icon: 'i-heroicons-envelope', color: 'success' })
      return
    }
    // Si pas de vérification requise, on peut tenter une connexion directe (optionnel)
    toast.add({ title: t('messages.account_created'), icon: 'i-heroicons-check-circle', color: 'success' })
  } catch (e: unknown) {
    const error = e as HttpError
