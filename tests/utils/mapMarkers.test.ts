...
import { createCustomMarkerIcon, getEditionStatus, type MarkerOptions } from '../../app/utils/mapMarkers'
...
describe('mapMarkers utils', () => {
  describe('createCustomMarkerIcon', () => {
    ...
    it('devrait gérer toutes les combinaisons de statut', () => {
      ...
      testCases.forEach(({ options, expectedColor, expectedStroke, expectedStrokeWidth }, index) => {
        vi.clearAllMocks()
    ...
    it('devrait générer un SVG valide', () => {
      ...
      expect(svgCall).toContain('<path d="M12.5 0C5.596 0 0 5.596 0 12.5c0 8.5 12.5 28.5 12.5 28.5s12.5-20 12.5-28.5C25 5.596 19.404 0 12.5 0z"')
  ...
  describe('getEditionStatus', () => {
    ...
    it('devrait gérer différents formats de date', () => {
      ...
      testCases.forEach(({ startDate, endDate, expectedUpcoming, expectedPast, expectedOngoing }) => {
    ...
    it('devrait maintenir la cohérence logique des statuts', () => {
      ...
      testDates.forEach(([start, end]) => {
        ...
        const trueCount = [status.isUpcoming, status.isOngoing, status.isPast].filter(Boolean).length
