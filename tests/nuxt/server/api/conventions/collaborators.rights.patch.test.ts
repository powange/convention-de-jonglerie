...
describe('/api/conventions/[id]/collaborators/[collaboratorId].rights PATCH', () => {
  ...
  const existingCollaborator = {
    ...
    perEditionPermissions: [
      { editionId: 100, canEdit: true, canDelete: false }
    ]
  };
  ...
  it('met à jour droits globaux et crée historique', async () => {
    ...
    prismaMock.conventionCollaborator.update.mockResolvedValue({ ...existingCollaborator, canEditConvention: true });
  ...
  it('met à jour perEdition et log PER_EDITIONS_UPDATED', async () => {
    ...
    prismaMock.editionCollaboratorPermission.create.mockImplementation(({ data }: any) => Promise.resolve({ ...data }));
    ...
    const res = await handler(baseEvent as any);
    expect(res.success).toBe(true);
    expect(prismaMock.editionCollaboratorPermission.deleteMany).toHaveBeenCalledWith({ where: { collaboratorId: 9 } });
    expect(prismaMock.editionCollaboratorPermission.create).toHaveBeenCalled();
    expect(prismaMock.collaboratorPermissionHistory.create.mock.calls[0][0].data.changeType).toBe('PER_EDITIONS_UPDATED');
  ...
  it('met à jour droits globaux + perEdition ensemble (changeType PER_EDITIONS_UPDATED)', async () => {
    ...
    prismaMock.conventionCollaborator.update.mockResolvedValue({ ...existingCollaborator, canDeleteConvention: true });
    prismaMock.editionCollaboratorPermission.deleteMany.mockResolvedValue({});
    prismaMock.editionCollaboratorPermission.create.mockImplementation(({ data }: any) => Promise.resolve({ ...data }));
    prismaMock.collaboratorPermissionHistory.create.mockResolvedValue({ id: 3 });

    (global.readBody as any).mockResolvedValue({ rights: { deleteConvention: true }, perEdition: [ { editionId: 150, canEdit: true } ] });
