...
describe('Image Upload Security Tests', () => {
  ...
  describe('validateImageFile - Type MIME Security', () => {
    ...
    it('devrait rejeter les fichiers exécutables', async () => {
      ...
      await expect(
        validateImageFile(maliciousFile, { prefix: 'test', destinationFolder: 'test' })
    ...
    it('devrait rejeter les scripts JavaScript', async () => {
      ...
      await expect(
        validateImageFile(jsFile, { prefix: 'test', destinationFolder: 'test' })
    ...
    it('devrait rejeter les fichiers PHP', async () => {
      ...
      await expect(
        validateImageFile(phpFile, { prefix: 'test', destinationFolder: 'test' })
      ).rejects.toThrow('Type de fichier non autorisé')
    })
    ...
    it('devrait rejeter les fichiers HTML', async () => {
      ...
      await expect(
        validateImageFile(htmlFile, { prefix: 'test', destinationFolder: 'test' })
      ).rejects.toThrow('Type de fichier non autorisé')
    })
    ...
    it('devrait rejeter les fichiers SVG (potentiel XSS)', async () => {
      ...
      await expect(
        validateImageFile(svgFile, { prefix: 'test', destinationFolder: 'test' })
      ).rejects.toThrow('Type de fichier non autorisé')
    })
    ...
    it("devrait accepter uniquement les types d'images sûrs", async () => {
      ...
      for (const type of safeTypes) {
        ...
        await expect(
          validateImageFile(safeFile, { prefix: 'test', destinationFolder: 'test' })
    ...
    it('devrait rejeter les types MIME falsifiés', async () => {
      ...
      await expect(
        validateImageFile(fakeImageFile, { prefix: 'test', destinationFolder: 'test' })
    ...
    it('devrait rejeter les fichiers sans type MIME', async () => {
      ...
      await expect(
        validateImageFile(noTypeFile, { prefix: 'test', destinationFolder: 'test' })
      ).rejects.toThrow('Type de fichier non autorisé')
    })
    ...
    it('devrait rejeter les fichiers avec type MIME vide', async () => {
      ...
      await expect(
        validateImageFile(emptyTypeFile, { prefix: 'test', destinationFolder: 'test' })
      ).rejects.toThrow('Type de fichier non autorisé')
    })
  })
  ...
  describe('validateImageFile - Size Security', () => {
    ...
    it('devrait rejeter les fichiers trop volumineux (DoS protection)', async () => {
      ...
      await expect(
        validateImageFile(hugeFie, { prefix: 'test', destinationFolder: 'test' })
    ...
    it('devrait accepter les fichiers de taille raisonnable', async () => {
      ...
      await expect(
        validateImageFile(normalFile, { prefix: 'test', destinationFolder: 'test' })
      ).resolves.toBeUndefined()
    })
    ...
    it('devrait accepter les fichiers vides (cas limite)', async () => {
      ...
      await expect(
        validateImageFile(emptyFile, { prefix: 'test', destinationFolder: 'test' })
      ).resolves.toBeUndefined()
    })
    ...
    it('devrait gérer les tailles négatives (protection contre corruption)', async () => {
      ...
      await expect(
        validateImageFile(negativeFile, { prefix: 'test', destinationFolder: 'test' })
  ...
  describe('Error Handling Security', () => {
    it("ne devrait pas exposer d'informations sensibles dans les erreurs", async () => {
      const mockEvent = {} as any
      global.readMultipartFormData.mockRejectedValue(
        new Error('Database connection failed with password 123456')
      )
    ...
    it("devrait logger les erreurs sans les exposer à l'utilisateur", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Erreur lors de l'upload d'image:", expect.any(Error))
