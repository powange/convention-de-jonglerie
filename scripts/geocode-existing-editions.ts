...
async function geocodeEditionWithRetry(edition: any, attempt: number = 1): Promise<{ success: boolean; coords?: { latitude: number; longitude: number } }> {
  try {
...
async function processBatch(editions: any[], batchIndex: number): Promise<{ success: number; errors: number }> {
