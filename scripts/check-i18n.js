...
function extractI18nKeysFromFile(filePath) {
  ...
  'authStore', 'editionStore', 'favoriteEditions', 'passwordState', 'registerState', 'emailState',
      ...
      'addForm', 'replyForm', 'newPostForm', 'currentLanguage', 'lang',
      'leaflet', 'activeService', 'service', 'category'
    ];
    ...
    const isJsProperty = !isValidI18nKey && jsObjectPatterns.some(pattern => 
      potentialKey.startsWith(pattern + '.') || potentialKey === pattern
    );
...
function extractHardcodedTexts(filePath) {
  ...
  while ((match = textBetweenTagsRegex.exec(template)) !== null) {
    ...
    if (text && 
        ...
        !/^(Icon|Ui|App|Base)[A-Z]/.test(text)) { // Exclure les préfixes de composants communs
...
async function main() {
  ...
  console.log(`${CYAN}Analyse de ${allFiles.length} fichiers (${vueFiles.length} Vue, ${tsFiles.length} TS, ${jsFiles.length} JS)...${RESET}\n`);
  ...
  const dynamicPrefixes = [...usedKeys].filter(key => key.startsWith('__DYNAMIC_PREFIX__')).map(key => key.replace('__DYNAMIC_PREFIX__', ''));
  const cleanUsedKeys = new Set([...usedKeys].filter(key => !key.startsWith('__DYNAMIC_PREFIX__')));
  ...
  if (!stepToRun || stepToRun === 3) {
    ...
    if (duplicates.length > 0) {
      ...
      duplicates.forEach(({ value, keys }) => {
        console.log(`  ${YELLOW}Valeur: "${value.substring(0, 50)}${value.length > 50 ? '...' : ''}"${RESET}`);
  ...
  if (!stepToRun || stepToRun === 4) {
    if (!stepToRun) console.log('');
    console.log(`${BOLD}4. Textes hardcodés dans les fichiers Vue (potentiellement non traduits)${RESET}`);
  ...
  if (!stepToRun) {
    ...
    console.log(`Fichiers analysés: ${allFiles.length} (${vueFiles.length} Vue, ${tsFiles.length} TS, ${jsFiles.length} JS)`);
