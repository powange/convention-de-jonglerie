...
class TranslationChecker {
  ...
  analyzeFile(langCode) {
    ...
    const completionRate = totalKeys > 0 ? ((totalKeys - todoCount) / totalKeys * 100).toFixed(1) : '100.0';
  ...
  generateReport() {
    ...
    const overallCompletion = totalKeys > 0 ? ((totalKeys - totalTodos) / totalKeys * 100).toFixed(1) : '100.0';
    ...
    if (incompleteLanguages.length > 0) {
      ...
      for (const stat of incompleteLanguages.slice(0, 3)) { // Max 3 langues pour éviter un affichage trop long
  ...
  showRecommendations(stats) {
    ...
    if (incompleteCount <= 3) {
      ...
      if (mostIncomplete.length > 0) {
        ...
        console.log(`   node scripts/mass-translator.js --lang "${mostIncomplete.join(',')}" --dry-run`);
      }
    }
