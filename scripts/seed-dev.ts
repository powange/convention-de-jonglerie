...
async function main() {
  ...
  const testUsers = [
    ...
    { email: 'charlie.diabolo@example.com', pseudo: 'CharlieDiab', nom: 'Rousseau', prenom: 'Charlie' },
  ...
  const conventions = [
    { name: 'Convention Nationale de Jonglerie', description: 'Rassemblement national des jongleurs.', authorId: user.id },
    { name: 'Festival Jongle & Cirque', description: 'Festival convivial avec scènes et ateliers.', authorId: superAdmin.id },
    { name: 'Convention Régionale de Jonglerie', description: 'Edition régionale, ambiance locale.', authorId: user.id },
    { name: 'Festival International du Cirque', description: 'Grand rendez-vous des artistes du monde entier.', authorId: superAdmin.id },
  ]
  ...
  for (const convDef of conventions) {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if (!edition) {
        ...
        console.log(`Edition créée: ${edition.name} (id=${edition.id}, ${startDate.toLocaleDateString()})`)
  ...
  const departureCities = ['Paris', 'Lyon', 'Marseille', 'Toulouse', 'Bordeaux', 'Lille', 'Nice', 'Nantes']
  ...
  for (const edition of createdEditions) {
    ...
    for (let i = 0; i < numOffers; i++) {
      ...
      const randomUser = i === 0 ? superAdmin : createdUsers[Math.floor(Math.random() * createdUsers.length)]
      ...
      if (Math.random() > 0.5) {
        ...
        for (let j = 0; j < numComments; j++) {
          const commentUser = createdUsers[Math.floor(Math.random() * createdUsers.length)]
          if (commentUser.id !== randomUser.id) { // Pas de commentaire de l'auteur sur sa propre offre
    ...
    for (let i = 0; i < numRequests; i++) {
      ...
      const randomUser = Math.random() < 0.3 ? superAdmin : createdUsers[Math.floor(Math.random() * createdUsers.length)]
      const departureCity = departureCities[Math.floor(Math.random() * departureCities.length)]
      ...
      const request = await prisma.carpoolRequest.create({
        data: {
          ...
          phoneNumber: Math.random() > 0.5 ? `06${Math.floor(Math.random() * 90000000) + 10000000}` : null,
        },
      })
