...
export default defineEventHandler(async (event) => {
  ...
  const requestId = parseInt(event.context.params?.id as string);

  if (!requestId) {
    throw createError({
      statusCode: 400,
      statusMessage: 'ID de la demande invalide',
    });
  }

  try {
    const comments = await prisma.carpoolRequestComment.findMany({
      where: {
        carpoolRequestId: requestId,
      },
      include: {
        user: true,
      },
