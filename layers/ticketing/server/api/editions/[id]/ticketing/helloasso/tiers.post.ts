...
export default wrapApiHandler(
  async (event) => {
    ...
    // Une seule synchronisation à la fois par édition. Deux appels concurrents — deux onglets,
    // un rechargement — écrivaient les mêmes lignes en parallèle et se bloquaient jusqu'au
    // « Lock wait timeout exceeded » de MySQL, en retenant chacun une connexion cinquante
    // secondes durant. Le second appel reçoit désormais le résultat du premier.
    return synchronisationUnique(`helloasso-tiers-${editionId}`, async () => {
      try {
