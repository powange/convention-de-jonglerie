...
/** Un groupe, réduit à ce dont le filtre a besoin. */
export interface GroupeDEmprunt {
  id: number
  name: string
}

/** Un tag, réduit à ce dont le filtre et la pastille ont besoin. */
export interface TagDEmprunt {
  id: number
  name: string
  color: string
}

/** Ce qu'un emprunt porte, en plus de son étape, pour ces deux filtres-ci. */
export interface EmpruntRangeable {
  group?: GroupeDEmprunt | null
  tags?: Array<{ tag: TagDEmprunt }> | null
}

/**
 * Les groupes réellement présents dans une liste d'emprunts.
 *
 * Même parti que `valeursDEtape` : une liste fermée, construite sur ce qui est sous les yeux,
 * plutôt que le catalogue entier de l'édition. Un groupe dont aucun objet n'est emprunté n'a rien
 * à proposer — le choisir ne viderait pas seulement le tableau, il ferait douter du filtre.
 *
 * Trié par nom : les identifiants ne disent rien à qui lit, et l'ordre de la base encore moins.
 */
export function groupesDesEmprunts<T extends EmpruntRangeable>(emprunts: T[]): GroupeDEmprunt[] {
  const parId = new Map<number, GroupeDEmprunt>()
  for (const emprunt of emprunts) {
    if (emprunt.group) parId.set(emprunt.group.id, emprunt.group)
  }
  return [...parId.values()].sort((a, b) => a.name.localeCompare(b.name, 'fr'))
}

/**
 * Les tags réellement posés sur les objets d'une liste d'emprunts.
 *
 * Un objet peut en porter plusieurs : chacun entre dans la liste, sans doublon. La couleur
 * voyage avec, parce que le sélecteur montre la pastille — reconnaître un tag à sa couleur est
 * précisément ce pour quoi on l'a posé.
 */
export function tagsDesEmprunts<T extends EmpruntRangeable>(emprunts: T[]): TagDEmprunt[] {
  const parId = new Map<number, TagDEmprunt>()
  for (const emprunt of emprunts) {
    for (const lien of emprunt.tags ?? []) {
...
/**
 * Les emprunts appartenant à l'un des groupes choisis.
 *
 * Aucun groupe choisi, rien n'est filtré — un filtre vide ne doit pas vider l'écran, comme pour
 * `filtrerParEtape`. La comparaison porte sur l'identifiant et non sur le nom : deux groupes
 * peuvent porter le même intitulé, et c'est le genre d'égalité qui se découvre tard.
 *
 ...
 */
export function filtrerParGroupes<T extends EmpruntRangeable>(
  emprunts: T[],
  groupeIds: readonly number[] | null | undefined
): T[] {
  if (!groupeIds?.length) return emprunts
  const retenus = new Set(groupeIds)
  return emprunts.filter((emprunt) => (emprunt.group ? retenus.has(emprunt.group.id) : false))
}
