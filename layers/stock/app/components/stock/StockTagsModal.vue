<!--
  Gestion des tags du stock : créer, renommer, recolorer, supprimer.

  Repris de la modale des tags de tâches, geste pour geste, à une différence près : les tags du
  stock appartiennent à l'ÉDITION et non au groupe. C'est la seule façon de retrouver le même
  « fragile » d'un groupe de rangement à l'autre sans le recréer partout.
-->
<template>
  <UModal
    v-model:open="isOpen"
    :title="$t('gestion.stock.tags.modal_title')"
    :ui="{ content: 'sm:max-w-xl' }"
  >
    <template #body>
      <div class="space-y-3">
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ $t('gestion.stock.tags.description') }}
        </p>
        ...
        <div class="pt-3 border-t border-gray-200 dark:border-gray-800 space-y-2">
          <h4 class="text-sm font-medium">{{ $t('gestion.stock.tags.new_tag') }}</h4>
          <div class="flex items-center gap-2">
            <UInput
              v-model="newTagName"
              :placeholder="$t('gestion.stock.tags.name_placeholder')"
              size="sm"
              class="flex-1"
...
async function addTag() {
  ...
  try {
    const res = await $fetch<{ success: boolean; data: { tag: TagItem } }>(
      `/api/editions/${props.editionId}/stock-tags`,
...
async function saveEdit(tag: TagItem) {
  ...
  try {
    const res = await $fetch<{ success: boolean; data: { tag: TagItem } }>(
      `/api/editions/${props.editionId}/stock-tags/${tag.id}`,
