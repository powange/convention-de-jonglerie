...
describe('/api/auth/resend-verification POST', () => {
  ...
  it("devrait utiliser l'URL de base de l'environnement si disponible", async () => {
    const originalEnv = process.env.NUXT_APP_SITE_URL
    process.env.NUXT_APP_SITE_URL = 'https://example.com'
    ...
    if (originalEnv) {
      process.env.NUXT_APP_SITE_URL = originalEnv
    } else {
      delete process.env.NUXT_APP_SITE_URL
    }
  })
})
